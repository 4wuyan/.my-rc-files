...
if string match --quiet '*YWU*' < /proc/sys/kernel/hostname
	...
	abbr --add --global update-conan-ips-server 'powershell.exe conan install --update --settings arch=x86_64 --settings build_type=Debug --settings compiler.runtime=MDd --settings compiler.toolset=v140 /Users/yan.wu/Music/ips-server/IpsServer'
	abbr --add --global update-conan-ips-server-release 'powershell.exe conan install --update --settings arch=x86_64 --settings build_type=Release --settings compiler.runtime=MD --settings compiler.toolset=v140 /Users/yan.wu/Music/ips-server/IpsServer'
end
